...
<template>
	<div class="mTable" :style="{ 'height': _height }" ref="mTableMain" v-loading="loading">
		<div class="mTable-table" :style="{ 'height': _table_height }">
		...
		<div class="mTable-page" v-if="!hidePagination || !hideDo">
			<div class="mTable-pagination">
				<el-pagination v-if="!hidePagination" background  :layout="paginationLayout" :total="total"
					:page-size="pageSize" :page-sizes="pageSizes" v-model:current-page="currentPage"
					@current-change="paginationChange" @update:page-size="pageSizeChange"></el-pagination>
			</div>
			<div class="mTable-do" v-if="!hideDo">
...
import { ref, reactive, watch, computed, onMounted, onActivated, onDeactivated, PropType,nextTick } from 'vue'
...
const props = defineProps({
	...
	api: { type: Function as PropType<(...args: any) => any>, default: null },
...
const getData = async () => {
	...
	mTableRef.value.setScrollTop(0)
...
const reload = (params: any, page = 1) => {
	...
	Object.assign(tableParams.value, params || {})
	mTableRef.value.clearSelection();
	mTableRef.value.clearSort()
	mTableRef.value.clearFilter()
	getData()
}
...
const toggleRowSelection = (row: any, selected: any) => {
	mTableRef.value.toggleRowSelection(row, selected)
}
const toggleAllSelection = () => {
	mTableRef.value.toggleAllSelection()
}
const toggleRowExpansion = (row: any, expanded: any) => {
	mTableRef.value.toggleRowExpansion(row, expanded)
}
const setCurrentRow = (row: any) => {
	mTableRef.value.setCurrentRow(row)
}
const clearSort = () => {
	mTableRef.value.clearSort()
}
const clearFilter = (columnKey: any) => {
	mTableRef.value.clearFilter(columnKey)
}
const doLayout = () => {
	mTableRef.value.doLayout()
}
const sort = (prop: any, order: any) => {
	mTableRef.value.sort(prop, order)
}
...
.nTable:deep(.el-table__footer) .cell {
	font-weight: bold;
}

.mTable:deep(.el-table__body-wrapper) .el-scrollbar__bar.is-horizontal {
	height: 12px;
	border-radius: 12px;
}

.mTable:deep(.el-table__body-wrapper) .el-scrollbar__bar.is-vertical {
	width: 12px;
	border-radius: 12px;
}
</style>